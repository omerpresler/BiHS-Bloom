...
public:
	...
	int numOfExp = 0;
	State rendezvous;
	EpisodicTASFrontier *other;
    int sampleCount;
	State anchor;
	State start;
    Heuristic<State> *h;
	EpisodicTASFrontier(Env *_env, State _start, Heuristic<State> *h, int _sampleCount);
	~EpisodicTASFrontier(){}
...
template <class Env,  class State>
EpisodicTASFrontier<Env, State>::EpisodicTASFrontier(Env *_env, State _start, Heuristic<State> *_h, int _sampleCount)
{
...
template <class Env,  class State>
void EpisodicTASFrontier<Env, State>::ExtractPath(std::vector<State> &path)
{
...
template <class Env,  class State>
bool EpisodicTASFrontier<Env, State>::DoSingleSearchStep()
{
	if (open.size() == 0)
	{
...
template <class Env,  class State>
void EpisodicTASFrontier<Env, State>::SetSeed(unsigned int seed)
{
	this->seed = seed;
}
...
public:
	int episod = 10;
	EpisodicTASFrontier<Env, State>* ff;
	EpisodicTASFrontier<Env, State>* bf;
	EpisodicTAS();
	EpisodicTAS(Env *_env, State _start, State _goal, Heuristic<State> *hf, Heuristic<State> *hb, int _sampleCount);
	~EpisodicTAS(){}
	void Init(Env *_env, State _start, State _goal, Heuristic<State> *hf, Heuristic<State> *hb, int _sampleCount)
	{
		ff = new EpisodicTASFrontier<Env, State>(_env, _start, hf, _sampleCount);
		bf = new EpisodicTASFrontier<Env, State>(_env, _goal, hb, _sampleCount);
		ff->other = bf;
		bf->other = ff;
	}
	...
	void GetPath(std::vector<State> &path)
	{
		...
		while (true)
		{
			if (count % episod == 0)
			{
				...
				ff->h = bf->h;
				//std::cout << "TEST: " << bf->h->HCost(ff->anchor, bf->anchor) << std::endl;
				std::cout << "Episod(" << episod << "): " << count / episod << std::endl;
			}
			count++;
...
template <class Env, class State>
EpisodicTAS<Env, State>::EpisodicTAS(Env *_env, State _start, State _goal, Heuristic<State> *hf, Heuristic<State> *hb, int _sampleCount)
{
	ff = new EpisodicTASFrontier<Env, State>(_env, _start, hf, _sampleCount);
	bf = new EpisodicTASFrontier<Env, State>(_env, _goal, hb, _sampleCount);
	ff->other = bf;
