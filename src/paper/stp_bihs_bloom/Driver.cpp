...
void DFS(MNPuzzle<MN_SIZE, MN_SIZE>& env,
         const MNPuzzleState<MN_SIZE, MN_SIZE>& curr,
         ...
         const MNPuzzleState<MN_SIZE, MN_SIZE>& parent)
{
    ...
    for (const auto& neighbor : neighbors) {
        ...
        DFS(env, neighbor, depth + 1, targetDepth, upperBound, goal, bf, existingBf, curr);
    }
}

BloomFilter* GetBloomOfStatesInBloomAtDepth(MNPuzzleState<MN_SIZE, MN_SIZE> start,
          ...
          int k_hashes)
{
    ...
    size_t upperBound = distance * 2 + 1; // Distance cant be bigger then half of the cost
    ...
    if (distance == 0) {
        if (existingBf == nullptr) {
            ...
            if (bloom_maybe_contains(existingBf, &start.puzzle, sizeof(start.puzzle))) {
                bloom_add(bf, &start.puzzle, sizeof(start.puzzle));
            }
        }
        return bf;
          ...
          int minItemsInserted)
{
    ...
    do {
        BloomFilter* nextBf;
        if (forward) {
            nextBf = GetBloomOfStatesInBloomAtDepth(start, goal, forwardDepth, bf, size_in_KiB, k_hashes);
        } else {
            nextBf = GetBloomOfStatesInBloomAtDepth(goal, start, backwardDepth, bf, size_in_KiB, k_hashes);
        }
        forward = !forward;
        ...
        // Safety break to prevent infinite loop if count doesn't increase (since depth is constant)
...
{   
    ...
    std::cout << "DEBUG benchmark entry: sizes[0]=" << sizes[0]
              << ", sizes[1]=" << sizes[1] << std::endl;

    for (int i = 0; i < 2; i++) {
        for (int j = 0; j < 2; j++) {

            std::cout << "DEBUG before solve: i=" << i << ", j=" << j
                      << ", sizes[i]=" << sizes[i] << std::endl;

            solve_at_depth(start, goal,
                           forwardDepth, backwardDepth,
                           sizes[i], k_hashes[j], 3);

            std::cout << "DEBUG after solve:  i=" << i << ", j=" << j
                      << ", sizes[i]=" << sizes[i] << std::endl;
        }
    }
    return 0;
                    ...
                    const MNPuzzleState<MN_SIZE, MN_SIZE> &start)
{
    ...
    while (!q.empty()) {
        ...
        if (cur.depth > distance) continue;
...
void printUsage(const char* progName) {
    std::cerr << "Usage:\n"
              ...
              << "  Benchmark: " << progName << " --benchmark -d <dist> -f <file>\n";
}
...
int main(int argc, char** argv)
{
    ...
    if ((generate && benchmarkMode) || (!generate && !benchmarkMode)) {
        std::cerr << "Error: Must specify exactly one of --generate or --benchmark.\n";
        printUsage(argv[0]);
        return 1;
    }
    ...
    if (generate) {
        ...
        for (size_t i = 0; i < puzzles.size(); ++i) {
            ...
            std::cout << "A* Nodes expanded: " << astar.GetNodesExpanded() << std::endl;
            ...
            benchmark(puzzles[i], goal, distance);
        }
    }
