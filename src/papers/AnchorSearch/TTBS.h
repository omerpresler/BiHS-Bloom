...
public:
    ...
    double h;
    bool reprioritized;
    TTBSData(State _state, State _d, double _h, int _iter, bool _reprioritized)
    {
        state = _state;
        d = _d;
        h = _h;
        iter = _iter;
        reprioritized = _reprioritized;
    }
...
public:
	...
	std::priority_queue<TTBSData<State>, std::vector<TTBSData<State>>, TTBSCompare<TTBSData<State>>> *open;
...
bool TTBSFrontier<Env, State>::DoSingleSearchStep()
{
    ...
    /*
    std::vector<TTBSData<State>> temp;
    temp.clear();
    while (!open->empty())
    {
	...
	*/
    ...
    while(true)
    {
        ...
        for (auto item : similars)
        {
            if (item == other->open->top().state)
            {
                ...
                for (auto neighbor : children)
                {
                    ...
                    {
                        gValues[nhash] = g;
			            open->push(TTBSData<State>(neighbor, other->open->top().state, HCost(neighbor, other->open->top().state), numOfExp, false));
			            parents[nhash] = s;
                    }
                }
                break;
        ...
        {
            open->push(TTBSData<State>(s, other->open->top().state, HCost(s, other->open->top().state), numOfExp, true));
        }
    }
    numOfExp++;
