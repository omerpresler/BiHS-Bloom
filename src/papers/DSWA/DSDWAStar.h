...
const char *policyName[] = {"Weighted A*", "pwXD", "pwXU", "XDP", "XUP", "DWP", "MAP", "Half Edge", "Greedy", "[error]"};
...
public:
	...
	void DrawAlternate(Graphics::Display &disp) const;
    ...
    double Phi_v2(double h, double g) const
    {
        if (policy == kXDP)
        {
                float nextF = (g+(2*weight-1)*h+sqrt((g-h)*(g-h)+4*weight*g*h))/(2*weight);
    ...
    **/
    float GetPriority(float h, float g) const
    {
        if (data.size() == 0)
        {
            if (fequal(g, 0))
	...
	/**
	* Returns the weight being used for a state given its h anf g values.
	* Iterates through the data, to find the region that covers the slope (g/h) of the given state.
	* @param h and @param g The h and g values of the state
	**/
	float GetStateWeight(float h, float g) const
	{
		float slope = g/h;
		if (fgreater(slope, data.back().slope))
		{
    ...
    * This is a new version that uses a vector lookup table
    **/
    float GetPriority_v2(float h, float g) const
    {
...
/**
 * Draw the open/closed list according to priority
 *
 */
template <class state, class action, class environment, class openList>
void DSDWAStar<state, action,environment,openList>::DrawAlternate(Graphics::Display &disp) const
{
	double transparency = 1.0;
	if (openClosedList.size() == 0)
		return;
	...
	{
		const auto &data = openClosedList.Lookat(x);
		float w = GetStateWeight(data.h, data.g);
		
		w = (w-1)/(2*weight-2);
		env->SetColor(w, 0.0, 1-w);
		env->Draw(disp, data.data);
	}
	env->SetColor(1.0, 0.5, 1.0, 0.5);
	env->Draw(disp, goal);
...
//    display.DrawLine(LocalToHOG(bl1), LocalToHOG(bl2), 1.5f/100.0f, Colors::yellow);
    // 45° upper bound line
    display.DrawLine(LocalToHOG(bl2), LocalToHOG(bl3), 1.5f/100.0f, Colors::lightgray);
    // 2w-1 upper bound line
    display.DrawLine(LocalToHOG(bl1), LocalToHOG(bl2c), 1.5f/100.0f, Colors::lightgray);

    // 45° lower bound line
    display.DrawLine(LocalToHOG(bl2a), LocalToHOG(bl1), 1.5f/100.0f, Colors::lightgray);
    // 2w-1 lower bound line
    display.DrawLine(LocalToHOG(bl2), LocalToHOG(bl4), 1.5f/100.0f, Colors::lightgray);
